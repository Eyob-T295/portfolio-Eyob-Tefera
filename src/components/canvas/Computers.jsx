...
const Computers = ({ isMobile }) => {
  ...
  // Fix upside-down screen texture: Three.js loads GLTF textures with flipY=false,
  // but our custom screen.png needs flipY=true to appear right-side-up on this mesh.
  useEffect(() => {
    ...
    computer.scene.traverse((obj) => {
      if (obj.isMesh && obj.material) {
        ...
        mats.forEach((mat) => {
          if (mat.name && mat.name.includes("Material.074_30")) {
            if (mat.map) {
              mat.map.flipY = true;
              mat.map.needsUpdate = true;
            }
            if (mat.emissiveMap) {
              mat.emissiveMap.flipY = true;
